<template>
    <div class="box">
        <van-notice-bar
                left-icon="volume-o"
                text="此页为淘宝监控数据,每日定时启动监测,如有上下架商品,需尽快处理,祝生意兴隆~"
        />
        ...
        <div class="content">
            <div class="list" v-for="item in list" :key="item.id" >
                <van-cell-group :title="item.createTime">
                    <van-cell title="扫描总个数" :value="item.total" />
                    <van-cell title="下架个数" :value="item.amyOfflineNum" />
                    <van-cell title="上架个数" :value="item.amyOnNum" />
                    <div style="text-align: center">
                        <van-button type="danger" v-if="item.isDo=='0'" size="small"  @click="doJob(item.id)" >已处理</van-button>
                    </div>
                </van-cell-group>
                </div>
        ...
        methods: {
            ...
            getList: async function(cp,c) {
               ...
               /* this.$toast.loading({
                ...
                const result = await monitorRecord(params);
                this.$toast.clear();
